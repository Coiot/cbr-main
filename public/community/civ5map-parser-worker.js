...
self.onmessage = (event) => {
  try {
    ...
    if (!(buffer instanceof ArrayBuffer)) {
      throw new Error('Missing map buffer.');
    }
    const result = parseCiv5Map(buffer);
    self.postMessage({ ok: true, result });
  } catch (error) {
    self.postMessage({
      ok: false,
      error: error && error.message ? error.message : 'Unable to parse map data.',
    });
  }
};
...
function parseUnits(
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    cursor += 4;
    const unitType =
      version >= 12 ? view.getUint32(cursor, true) : view.getUint8(cursor);
